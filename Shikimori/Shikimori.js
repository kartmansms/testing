(function () {
  ...
  function search(animeData) {
    ...
    // Используем данные Shikimori напрямую
    console.log('Используем предоставленные данные Shikimori для аниме с ID:', animeData.id);
    processResults(animeData, animeData.kind);
  }
  ...
  function processResults(response, kind) {
    ...
    if (!response || !response.id) {
      console.error('Ответ пустой или отсутствует ID');
      Lampa.Noty.show('Не удалось найти аниме: некорректные данные');
      return;
    }

    // Проверяем, является ли ответ результатом поиска TMDB (для обратной совместимости)
    if ('total_results' in response) {
      if (response.total_results === 0) {
        console.warn('Результатов не найдено в поиске TMDB');
        Lampa.Noty.show('Не удалось найти аниме в TMDB');
      } else if (response.total_results === 1 && kind !== 'ona') {
        console.log('Найден один результат TMDB:', JSON.stringify(response.results[0], null, 2));
        if (!response.results[0].id || !response.results[0].media_type) {
          console.error('Некорректные данные в результате TMDB:', response.results[0]);
        ...
        console.log('Найдено несколько результатов TMDB:', response.results.length);
        var menu = [];
        response.results.forEach(function (item) {
          if (!item.id || !item.media_type) {
            console.warn('Пропущен элемент с некорректными данными TMDB:', JSON.stringify(item, null, 2));
            return;
          }
          menu.push({
        ...
        if (menu.length === 0) {
          console.error('Все элементы TMDB некорректны');
      ...
      // Обрабатываем данные Shikimori напрямую
      console.log('Прямой результат Shikimori:', JSON.stringify(response, null, 2));
      const shikimoriCard = {
        id: response.id, // Shikimori ID
        title: response.name || response.russian || response.english || response.japanese,
        media_type: kind === 'movie' ? 'movie' : 'tv', // Упрощённое определение типа
        poster_path: response.poster ? response.poster.originalUrl : '',
        release_date: response.airedOn && response.airedOn.year ? `${response.airedOn.year}-01-01` : null,
        shikimori_data: response // Сохраняем полные данные Shikimori
      };

      if (kind !== 'ona') {
        console.log('Запуск активности для Shikimori результата:', shikimoriCard);
        Lampa.Activity.push({
          url: '',
          component: 'full',
          id: shikimoriCard.id,
          method: shikimoriCard.media_type,
          card: shikimoriCard
        });
      } else {
        console.log('Тип ONA: отображение как одиночный результат');
        Lampa.Activity.push({
          url: '',
          component: 'full',
          id: shikimoriCard.id,
