(function () {
  ...
  // Функция для получения топ-100 ТВ-сериалов
  function getTop100(oncomplite, onerror) {
    $(document).ready(function () {
      var query = `
        query Animes {
          animes(limit: 100, order: "ranked", kind: "tv") {
            id
            name
            russian
            licenseNameRu
      ...
      $.ajax({
        ...
        error: function error(_error) {
          console.error('Ошибка при получении топ-100:', _error);
          onerror(_error);
        }
      });
    });
  ...
  function Component$1(object) {
    ...
    var head = $("<div class='Shikimori-head torrent-filter'><div class='Shikimori__home simple-button simple-button--filter selector'>Главная</div><div class='Shikimori__search simple-button simple-button--filter selector'>Фильтр</div><div class='Shikimori__top100 simple-button simple-button--filter selector'>Топ 100</div></div>");
    var body = $('<div class="Shikimori-catalog--list category-full"></div>');
    var active, last;

    this.create = function () {
      if (object.top100) {
        API.getTop100(this.build.bind(this), this.empty.bind(this));
      } else {
