(function () {
  ...
  function search(animeData) {
    ...
    function getTmdb(id, type, callback) {
      ...
      var apiUrlTMDB = 'https://api.themoviedb.org/3/';
      var apiUrlProxy = 'apitmdb.' + (Lampa.Manifest && Lampa.Manifest.cub_domain ? Lampa.Manifest.cub_domain : 'cub.red') + '/3/';
      var request = "".concat(type, "/").concat(id, "?api_key=").concat(apiKey, "&language=").concat(Lampa.Storage.field('language'), "&append_to_response=seasons");
      var url = Lampa.Storage.field('proxy_tmdb') ? Lampa.Utils.protocol() + apiUrlProxy + request : apiUrlTMDB + request;
    ...
    function processResults(response) {
      ...
      if (response.total_results !== undefined) {
        if (response.total_results === 0) {
          ...
          if (item.media_type === 'tv' && item.seasons && item.seasons.length > 1) {
            showSeasonSelection(item);
          } else {
            Lampa.Activity.push({
              url: '',
              component: 'full',
          ...
          Lampa.Select.show({
            ...
            onSelect: function onSelect(a) {
              if (a.card.media_type === 'tv' && a.card.seasons && a.card.seasons.length > 1) {
                showSeasonSelection(a.card);
              } else {
                Lampa.Activity.push({
                  url: '',
                  component: 'full',
    ...
    function showSeasonSelection(card) {
      var seasonsMenu = [];
      card.seasons.forEach(function (season) {
        if (season.season_number >= 0) { // Исключаем специальные эпизоды (season_number = 0), если не нужны
          seasonsMenu.push({
      ...
      Lampa.Select.show({
        ...
        onSelect: function onSelect(a) {
          Lampa.Activity.push({
            ...
            method: 'tv',
            card: a.card,
            season: a.season_number // Добавляем информацию о выбранном сезоне
          });
        }
      });
    }
