...
(function () {
    ...
    function ownKeys(object, enumerableOnly) {
        // Получаем обычные ключи объекта
        var keys = Object.keys(object);
        // Проверяем, поддерживает ли объект символические ключи
        if (Object.getOwnPropertySymbols) {
            var symbols = Object.getOwnPropertySymbols(object);
            // Фильтруем только перечисляемые символы, если установлен флаг enumerableOnly
            if (enumerableOnly) {
                symbols = symbols.filter(function (symbol) {
                    return Object.getOwnPropertyDescriptor(object, symbol).enumerable;
                });
            }
            // Добавляем символы в массив ключей
    ...
    function _objectSpread2(target) {
        ...
        for (var index = 1; index < arguments.length; index++) {
            ...
            // Если индекс нечетный, используем только перечисляемые свойства
            if (index % 2) {
                ownKeys(Object(source), true).forEach(function (key) {
                    _defineProperty(target, key, source[key]);
                });
            } else if (Object.getOwnPropertyDescriptors) {
                // Используем Object.defineProperties для всех дескрипторов свойств
    ...
    function _regeneratorRuntime() {
        ...
        try {
            // Проверка, поддерживает ли браузер прямое определение свойств
            define({}, '');
        } catch (error) {
...
// Новая функция нормализации имени
  ...
  function mapAniListToTmdb(media, kind, year) {
    ...
    function Card(data, userLanguage) {
        ...
        var item = Lampa.Template.get("Shikimori-Card", {
            ...
            rate: data.score,
            title: userLanguage === 'ru' ? data.russian || data.name || data.japanese : data.name || data.japanese,
    ...
    // Основной компонент для отображения каталога (оставляем без изменений)
    function Component$1(object) {
        var userLanguage = Lampa.Storage.field('language');
        ...
        this.headeraction = function () {
            ...
            function queryForShikimori() {
                var query = {};
                filters.AnimeKindEnum.items.forEach(function (item) {
                    if (item.selected) query.kind = item.code;
                });
                filters.status.items.forEach(function (item) {
                    if (item.selected) query.status = item.code;
                });
                filters.kind.items.forEach(function (item) {
                    if (item.selected) query.genre = item.id;
                });
                filters.sort.items.forEach(function (item) {
                    if (item.selected) query.sort = item.code;
                });
                filters.seasons.items.forEach(function (item) {
                    if (item.selected) query.seasons = item.code;
                });
                return query;
            }
            ...
            function selected(where) {
                var titles = [];
                where.items.forEach(function (item) {
                    if (item.selected || item.checked) titles.push(item.title);
                });
                where.subtitle = titles.length ? titles.join(', ') : Lampa.Lang.translate('nochoice');
            }

            function select(where, item) {
            ...
            function mainMenu() {
                for (var key in filters) selected(filters[key]);
                Lampa.Select.show({
                    title: 'Фильтры',
                    items: [{
                    ...
                    onSelect: function (selected) {
                        if (selected.searchShikimori) {
                            search();
                        } else submenu(selected, mainMenu);
                    }
                });
            }
        ...
        this.pause = function () { };
        this.stop = function () { };
        this.render = function (javascript) {
    ...
    // Компонент для расширения информации в карточке (оставляем без изменений)
    function Component() {
