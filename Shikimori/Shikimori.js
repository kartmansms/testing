(function () {
  ...
  if (typeof Lampa === 'undefined') {
    console.error('Lampa не определен');
    return;
  }

  if (typeof Symbol === 'undefined') {
    console.error('Symbol не поддерживается');
    return;
  }

  if (typeof Object.getOwnPropertySymbols === 'undefined') {
    console.error('Object.getOwnPropertySymbols не поддерживается');
    return;
  }

  if (typeof Object.defineProperty === 'undefined') {
    console.error('Object.defineProperty не поддерживается');
    return;
  }

  // Уникальная функция ownKeys
  ...
  function translateStatus(status) {
    switch (status.toLowerCase()) {
      ...
      case 'latest':
        return 'Последнее';
      default:
        return status.charAt(0).toUpperCase() + status.slice(1); // Если статус неизвестен, возвращаем его без изменений
    }
  }
  ...
  var API = {
    main: function (params, oncomplite, onerror) {
      $(document).ready(function () {
        var query = `
          query Animes {
            animes(limit: ${params.limit || 36}, order: ${params.sort || 'ranked'}, page: ${params.page}) {
              id
              name
              russian
              licenseNameRu
        ...
        $.ajax({
          ...
          success: function (response) {
            if (response.errors) {
              console.error('Ошибка в ответе:', response.errors);
  ...
  function searchTmdb(query, callback) {
    ...
    var request = `search/multi?api_key=${apiKey}&language=${Lampa.Storage.field('language')}&include_adult=true&query=${cleanName(query)}`;
    $.get(Lampa.Storage.field('proxy_tmdb') ? Lampa.Utils.protocol() + apiUrlProxy + request : apiUrlTMDB + request, callback);
  }

  // Функция для получения данных с TMDB по ID
  function getTmdb(id, type, callback) {
    var apiKey = "4ef0d7355d9ffb5151e987764708ce96";
    var apiUrlTMDB = 'https://api.themoviedb.org/3/';
    var apiUrlProxy = 'apitmdb.' + (Lampa.Manifest && Lampa.Manifest.cub_domain ? Lampa.Manifest.cub_domain : 'cub.red') + '/3/';
    var request = `${type}/${id}?api_key=${apiKey}&language=${Lampa.Storage.field('language')}`;
    $.get(Lampa.Storage.field('proxy_tmdb') ? Lampa.Utils.protocol() + apiUrlProxy + request : apiUrlTMDB + request, callback);
  }
  ...
  // Обработка ответа с японским именем
  function handleFallbackResponse(fallbackResponse) {
    if (!fallbackResponse || fallbackResponse.total_results === 0) {
    ...
    if (response.total_results !== undefined) {
      if (response.total_results === 0) {
        ...
        response.results.forEach(function (animeItem) {
          menu.push({
            title: `[${animeItem.media_type.toUpperCase()}] ${animeItem.name ? animeItem.name : animeItem.title}`,
            card: animeItem
          });
        });
        Lampa.Select.show({
    ...
    this.headeraction = function () {
      ...
      $.ajax(settings).done(function (response) {
        ...
        var modifiedResponse = filteredResponse.map(function (item) {
          return { ...item, title: translateGenre(item.name), name: undefined };
        });
        filters.kind = { title: 'Жанр', items: modifiedResponse };
      });

      filters.AnimeKindEnum = { title: 'Тип', items: [
        { title: "TV Сериал", code: "tv" },
        { title: "Фильм", code: "movie" },
        { title: "OVA", code: "ova" },
        { title: "ONA", code: "ona" },
        { title: "Спешл", code: "special" },
        { title: "TV Спешл", code: "tv_special" },
        { title: "Музыка", code: "music" },
        { title: "PV", code: "pv" },
        { title: "CM", code: "cm" }
      ]};

      filters.status = { title: 'Статус', items: [
        { title: "Анонс", code: "anons" },
        { title: "Онгоиг", code: "ongoing" },
        { title: "Вышло", code: "released" }
      ]};

      filters.sort = { title: 'Сортировка', items: [
        { title: "По рейтингу", code: "ranked" },
        { title: "По популярности", code: "popularity" },
        { title: "По алфавиту", code: "name" },
        { title: "По дате выхода", code: "aired_on" },
        { title: "По типу", code: "kind" },
        { title: "По количеству эпизодов", code: "episodes" },
        { title: "По статусу", code: "status" },
        { title: "По рейтингу Shikimori", code: "ranked_shiki" }
      ]};
      ...
      anonsElement.on('hover:enter', function () {
        Lampa.Activity.push({ url: '', title: 'Shikimori', component: 'Shikimori', page: 1, limit: 100, sort: 'ranked' });
      });
    };
  ...
  function startPlugin() {
    ...
    Lampa.Template.add('ShikimoriStyle', "<style>.Shikimori-catalog--list.category-full{-webkit-box-pack:justify !important;-webkit-justify-content:space-between !important;-ms-flex-pack:justify !important;justify-content:space-between !important}.Shikimori-head.torrent-filter{margin-left:1.5em}.Shikimori.card__type{background:#ff4242;color:#fff}.Shikimori .card__season{position:absolute;left:-0.8em;top:3.4em;padding:.4em .4em;background:#05f;color:#fff;font-size:.8em;-webkit-border-radius:.3em;border-radius:.3em}.Shikimori .card__status{position:absolute;left:-0.8em;bottom:1em;padding:.4em .4em;background:#ffe216;color:#000;font-size:.8em;-webkit-border-radius:.3em;border-radius:.3em}.Shikimori.card__season.no-season{display:none}</style>");
    Lampa.Template.add("Shikimori-Card", "<div class=\"Shikimori card selector layer--visible layer--render\"><div class=\"Shikimori card__view\"><img src=\"{img}\" class=\"Shikimori card__img\" /><div class=\"Shikimori card__type\">{type}</div><div class=\"Shikimori card__vote\">{rate}</div><div class=\"Shikimori card__season\">{season}</div><div class=\"Shikimori card__status\">{status}</div></div><div class=\"Shikimori card__title\">{title}</div></div>");
