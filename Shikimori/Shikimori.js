(function () {
  ...
  function translateSeason(season) {
    ...
    const seasonName = parts[0].toLowerCase(); // Название сезона (например, "spring")
    const year = parts[1]; // Год (например, "2009")
  ...
  // Основная функция для запроса данных с Shikimori
  function main(params, onComplete, onError) {
    $(document).ready(function () {
      // Начинаем формировать запрос с базовыми параметрами
      let query = `
        query Animes {
          animes(limit: ${params.limit || 36}, order: ${params.sort || 'ranked'}, page: ${params.page || 1}`;
  ...
  function search(animeData) {
    function cleanName(name) {
      const regex = /\b(Season|Part)\s*\d*\.?\d*\b/gi;
      return name.replace(regex, '').trim().replace(/\s{2,}/g, ' ');
    }

    // Первый запрос к API для получения ID MyAnimeList
  ...
  function Card(data, userLang) {
    const formattedSeason = data.season ? translateSeason(data.season) : '';
  ...
  function Component$1(object) {
    ...
    const head = $("<div class='Shikimori-head torrent-filter'><div class='Shikimori__home simple-button simple-button--filter selector'>Главная</div><div class='Shikimori__anons simple-button simple-button--filter selector'>Топ 100</div><div class='Shikimori__search simple-button simple-button--filter selector'>Фильтр</div></div>");
    ...
    this.headeraction = function () {
      ...
      $.ajax(settings).done(function (response) {
        const filteredResponse = response.filter(item => item.entry_type === "Anime");
        const modifiedResponse = filteredResponse.map(item => ({
          ...item,
          title: translateGenre(item.name),
          name: undefined
        }));
        filters.kind = {
          title: 'Жанр',
      ...
      const homeElement = head.find('.Shikimori__home');
      ...
      const anonsElement = head.find('.Shikimori__anons');
  ...
  function add() {
    const button = $(`<li class="menu__item selector">
      <div class="menu__ico">
        <svg fill="currentColor" viewBox="0 0 24 24" role="img" xmlns="http://www.w3.org/2000/svg" stroke="">
          <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
          <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
          <g id="SVGRepo_iconCarrier">
            <title>Shikimori icon</title>
