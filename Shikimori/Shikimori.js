(function () {
  ...
  function Component$1(object) {
    ...
    var head = $("<div class='Shikimori-head torrent-filter'><div class='Shikimori__home simple-button simple-button--filter selector'>Главная</div><div class='Shikimori__search simple-button simple-button--filter selector'>Фильтр</div><div class='Shikimori__popular simple-button simple-button--filter selector'>Most Popular</div><div class='Shikimori__airing simple-button simple-button--filter selector'>Top Airing</div></div>");
    var body = $('<div class="Shikimori-catalog--list category-full"></div>');
    var active, last;

    this.create = function () {
      if (object.data) {
        this.build(object.data); // Если данные уже есть (Most Popular/Top Airing)
      } else {
        API.main(object, this.build.bind(this), this.empty.bind(this)); // Обычный запрос к Shikimori
      }
    };
    ...
    this.build = function (result) {
      ...
      scroll.onEnd = function () {
        object.page++;
        if (object.data) {
          // Для Most Popular и Top Airing используем Jikan API
          fetchJikanData(object.title.includes('Popular') ? 'bypopularity' : 'airing', _this.build.bind(_this));
        } else {
    ...
    this.headeraction = function () {
      ...
      var popularElement = head.find('.Shikimori__popular');
      var airingElement = head.find('.Shikimori__airing');
      ...
      // Функция для получения данных с Jikan API
      function fetchJikanData(type, callback) {
        var url = `https://api.jikan.moe/v4/top/anime?filter=${type}&limit=36&page=${object.page || 1}`;
        $.ajax({
          url: url,
          method: 'GET',
          success: function (response) {
            var animes = response.data.map(function (item) {
              return {
                id: item.mal_id,
                name: item.title,
                russian: item.title, // Русского названия нет в Jikan, используем английское
                japanese: item.title_japanese,
                kind: item.type.toLowerCase(),
                score: item.score,
                status: item.status.toLowerCase().includes('finished') ? 'released' : 'ongoing',
                season: item.season ? `${item.season}_${item.year}` : null,
                airedOn: { year: item.year || null },
                poster: { originalUrl: item.images.jpg.large_image_url }
              };
            });
            callback(animes);
          },
          error: function (error) {
            console.error(`Ошибка при запросе ${type} с Jikan API:`, error);
            this.empty();
          }.bind(this)
        });
      }

      popularElement.on('hover:enter', function () {
        fetchJikanData('bypopularity', function (animes) {
          Lampa.Activity.push({
            url: '',
            title: 'Most Popular (MyAnimeList)',
            component: 'Shikimori',
            page: 1,
            data: animes
          });
      ...
      airingElement.on('hover:enter', function () {
        fetchJikanData('airing', function (animes) {
          Lampa.Activity.push({
            url: '',
            title: 'Top Airing (MyAnimeList)',
            component: 'Shikimori',
            page: 1,
            data: animes
          });
