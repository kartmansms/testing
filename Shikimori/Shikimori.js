...
function ownKeys(obj, onlyEnumerable) { 
  ...
  function _objectSpread2(target, ...sources) {
    for (const source of sources) {
        if (!source) continue; // Пропускаем null и undefined

        const descriptors = Object.getOwnPropertyDescriptors
            ? Object.getOwnPropertyDescriptors(source)
            : ownKeys(source).reduce((acc, key) => {
  ...
  function _regeneratorRuntime() {
    if (typeof _regeneratorRuntimeInstance !== "undefined") {
        return _regeneratorRuntimeInstance;
    }

    const SymbolIterator = typeof Symbol === "function" && Symbol.iterator ? Symbol.iterator : "@@iterator";
    const SymbolAsyncIterator = typeof Symbol === "function" && Symbol.asyncIterator ? Symbol.asyncIterator : "@@asyncIterator";
    const SymbolToStringTag = typeof Symbol === "function" && Symbol.toStringTag ? Symbol.toStringTag : "@@toStringTag";
    ...
    const GeneratorPrototype = Object.create({
        [SymbolIterator]: function () {
            return this;
        }
    });

    function wrap(generatorFunction, GeneratorConstructor) {
        const proto = GeneratorConstructor?.prototype instanceof GeneratorFunction ? GeneratorConstructor : GeneratorFunction;
        const instance = Object.create(proto.prototype);
        instance._invoke = makeInvokeMethod(generatorFunction);
        return instance;
    }

    function makeInvokeMethod(generatorFunction) {
        ...
        return function invoke(method, arg) {
            ...
            try {
                const result = generatorFunction(method, arg);
                state = result.done ? "completed" : "suspendedYield";
                return result;
            } catch (error) {
                state = "completed";
                throw error;
    ...
    define(GeneratorPrototype, SymbolToStringTag, "Generator");
    define(GeneratorFunctionPrototype.prototype, "constructor", GeneratorFunctionPrototype);
    define(GeneratorFunction.prototype, "constructor", GeneratorFunction);

    _regeneratorRuntimeInstance = {
        wrap,
        mark: function (fn) {
            Object.setPrototypeOf(fn, GeneratorFunctionPrototype);
            fn.prototype = Object.create(GeneratorPrototype);
            return fn;
        },
        async: function (fn) {
  ...
  function _toPrimitive(value, hint) {
    ...
    if (typeof toPrim === "function") {
        ...
        throw new TypeError("Symbol.toPrimitive должен возвращать примитивное значение.");
    }

    if (hint === "string") {
        return value.toString !== Object.prototype.toString ? value.toString() : String(value);
    } else if (hint === "number") {
        return value.valueOf !== Object.prototype.valueOf ? value.valueOf() : Number(value);
    } else {
        return value.valueOf !== Object.prototype.valueOf ? value.valueOf() 
             : value.toString !== Object.prototype.toString ? value.toString() 
             : String(value);
    }
}
  ...
  function _toPropertyKey(value) {
    const key = _toPrimitive(value, "string");
  ...
  function asyncGeneratorStep(gen, resolve, reject, _next, _throw, key, arg) {
    try {
        if (typeof gen[key] !== "function") {
            throw new TypeError(`Invalid generator method: ${key}`);
        }

        const { value, done } = gen[key](arg);
        done ? resolve(value) : Promise.resolve(value).then(_next, _throw);
    } catch (error) {
        reject(error);
    }
}
  ...
  function _asyncToGenerator(fn) {
    return (...args) => {
        if (typeof fn !== "function") {
            return Promise.reject(new TypeError("_asyncToGenerator expects a function"));
        }
        ...
        return new Promise((resolve, reject) => {
            ...
            if (!gen || typeof gen.next !== "function") {
                return reject(new TypeError("Provided function must return a generator"));
            }

            const _next = (value) => asyncGeneratorStep(gen, resolve, reject, _next, _throw, "next", value);
            const _throw = (err) => asyncGeneratorStep(gen, resolve, reject, _next, _throw, "throw", err);
 ...
 function _toConsumableArray(arr) {
  ...
  if (typeof arr === 'string' || (typeof Symbol !== 'undefined' && arr[Symbol.iterator])) {
    return Array.from(arr); // Преобразуем итерируемые объекты и строки в массив
  }

  if (arr && typeof arr === 'object') {
    const type = Object.prototype.toString.call(arr).slice(8, -1);
    if (type === 'Map' || type === 'Set' || type === 'Arguments' || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(type)) {
      return Array.from(arr); // Преобразуем Map, Set, Arguments и типизированные массивы
    }
  }
