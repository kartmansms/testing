(function () {
  ...
  // Вспомогательные функции для работы с объектами (оставлены без изменений)
  ...
  // Полифил для генераторов (оставлен без изменений)
  ...
  // Функции для авторизации через Shikimori
  ...
  function startAuthorization() {
    const clientId = 'YOUR_CLIENT_ID'; // Замените на ваш client_id от Shikimori
    const redirectUri = encodeURIComponent(window.location.origin + '/callback');
    const codeVerifier = generateRandomString(128);
    localStorage.setItem('code_verifier', codeVerifier);

    generateCodeChallenge(codeVerifier).then(codeChallenge => {
      const authUrl = `https://shikimori.one/oauth/authorize?client_id=${clientId}&redirect_uri=${redirectUri}&response_type=code&code_challenge=${codeChallenge}&code_challenge_method=S256`;
  ...
  function handleCallback() {
    ...
    if (code) {
      ...
      $.ajax({
        ...
        data: `grant_type=authorization_code&client_id=${clientId}&redirect_uri=${redirectUri}&code=${code}&code_verifier=${codeVerifier}`,
  ...
  function getUserAnimeList(status, callback) {
    ...
    $.ajax({
      url: `https://shikimori.one/api/users/${userId}/anime_rates?status=${status}`,
      method: 'GET',
      headers: {
      ...
      error: function (error) {
        ...
        Lampa.Noty.show('Ошибка загрузки списка аниме');
      }
    });
  }
  ...
  // Поиск аниме через внешние API (оставлен без изменений)
  ...
  // Класс для карточки аниме (оставлен без изменений)
  ...
  // Компонент каталога (оставлен без изменений)
  function Component$1(object) {
    ...
    var head = $("<div class='Shikimori-head torrent-filter'><div class='Shikimori__home simple-button simple-button--filter selector'>Главная</div><div class='Shikimori__top100 simple-button simple-button--filter selector'>Топ100</div><div class='Shikimori__search simple-button simple-button--filter selector'>Фильтр</div></div>");
  ...
  function ProfileComponent(object) {
    var userLang = Lampa.Storage.field('language');
    var network = new Lampa.Reguest();
    var scroll = new Lampa.Scroll({
      mask: true,
    ...
    var items = [];
    var html = $("<div class='Shikimori-profile'></div>");
    var head = $("<div class='Shikimori-head torrent-filter'><div class='Shikimori__planned simple-button simple-button--filter selector'>Запланировано</div><div class='Shikimori__watching simple-button simple-button--filter selector'>Смотрю</div><div class='Shikimori__completed simple-button simple-button--filter selector'>Просмотрено</div><div class='Shikimori__dropped simple-button simple-button--filter selector'>Брошено</div></div>");
    var body = $('<div class="Shikimori-catalog--list category-full"></div>');
    var active, last;
    ...
    this.build = function (status) {
      ...
      this.headeraction();
      getUserAnimeList(status, function (data) {
        _this.body(data.map(item => item.anime));
      });
      scroll.append(head);
      scroll.append(body);
      html.append(scroll.render(true));
    ...
    this.headeraction = function () {
      var _this = this;
      head.find('.Shikimori__planned').on('hover:enter', function () {
        body.empty();
        items = [];
        _this.build('planned');
      });
      head.find('.Shikimori__watching').on('hover:enter', function () {
        body.empty();
        items = [];
        _this.build('watching');
      });
      head.find('.Shikimori__completed').on('hover:enter', function () {
        body.empty();
        items = [];
        _this.build('completed');
      });
      head.find('.Shikimori__dropped').on('hover:enter', function () {
        body.empty();
        items = [];
        _this.build('dropped');
      });
  ...
  // Компонент для расширения информации в карточке (оставлен без изменений)
  function Component() {
  ...
  function add() {
    ...
    $(".menu .menu__list").eq(0).append(shikimoriButton);
    $(".menu .menu__list").eq(0).append(profileButton);
  }
  ...
  function startPlugin() {
    ...
    var manifest = {
      ...
      version: "1.0",
      name: "LKE Shikimori",
      description: "Добавляет каталог Shikimori и профиль пользователя",
      component: "Shikimori"
    };
    ...
    Lampa.Component.add('Shikimori', Component$1);
    Lampa.Component.add('ShikimoriProfile', ProfileComponent);
    Component();
