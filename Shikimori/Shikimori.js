(function () {
  ...
  function _regeneratorRuntime() {
    ...
    try { define({}, ""); } catch (t) {
      define = function (t, e, r) { return t[e] = r; };
    }
      ...
      function invoke(r, o, i, a) {
        ...
        if ("throw" !== c.type) {
          ...
          return h && "object" == typeof h && n.call(h, "__await") ? e.resolve(h.__await).then(function (t) {
            invoke("next", t, i, a);
          }, function (t) { invoke("throw", t, i, a); }) : e.resolve(h).then(function (t) {
            u.value = t, i(u);
    ...
    return GeneratorFunction.prototype = GeneratorFunctionPrototype,
      ...
      e.awrap = function (t) { return { __await: t }; },
      defineIteratorMethods(AsyncIterator.prototype),
      define(AsyncIterator.prototype, c, function () { return this; }),
      ...
      define(g, a, function () { return this; }),
  ...
  function main(params, oncomplite, onerror) {
    $(document).ready(function () {
      ...
      $.ajax({
        ...
        success: function success(response) { oncomplite(response.data.animes); },
  ...
  // Поиск информации об аниме через внешние API, включая hanime.tv
  function search(animeData) {
    ...
    $.get("https://arm.haglund.dev/api/v2/ids?source=myanimelist&id=".concat(animeData.id), function (response) {
      if (response === null || response.themoviedb === null) {
        console.log('Поиск через TMDB по имени');
        searchTmdb(animeData.name, function (tmdbResponse) {
          handleTmdbResponse(tmdbResponse, animeData.japanese, menu);
        });
      } else {
        console.log('Получение данных TMDB по ID:', response.themoviedb);
        getTmdb(response.themoviedb, animeData.kind, function(tmdbResponse) {
          processResults(tmdbResponse, menu);
        });
      }
    }).fail(function (jqXHR) {
      if (jqXHR.status === 404) {
        ...
        Lampa.Noty.show('Ошибка при запросе к ARM: ' + jqXHR.status);
        console.error('Ошибка ARM:', jqXHR.status, jqXHR.responseText);
      }
    });

    // Поиск через hanime.tv
    API.searchHanime(animeData, function(hanimeResult) {
      if (hanimeResult) {
        menu.push({
          title: `[HANIME] ${hanimeResult.title}`,
          url: hanimeResult.url,
          source: 'hanime'
        });
      }
    ...
    function searchTmdb(query, callback) {
      ...
      $.get(url, callback).fail(function (jqXHR) {
        ...
        console.error('Ошибка TMDB:', jqXHR.status, jqXHR.responseText);
      });
    }

    function getTmdb(id, type, callback) {
      type = type && ['tv', 'movie', 'ova', 'ona', 'special', 'tv_special'].includes(type) ? 'tv' : 'movie';
      var apiKey = "4ef0d7355d9ffb5151e987764708ce96";
      var apiUrlTMDB = 'https://api.themoviedb.org/3/';
      var apiUrlProxy = 'apitmdb.' + (Lampa.Manifest && Lampa.Manifest.cub_domain ? Lampa.Manifest.cub_domain : 'cub.red') + '/3/';
    ...
    function handleTmdbResponse(tmdbResponse, fallbackQuery, menuRef) {
      if (tmdbResponse.total_results === 0) {
        Lampa.Noty.show('Результатов в TMDB не найдено, пробую японское название');
        searchTmdb(fallbackQuery, function(fallbackResponse) {
          processResults(fallbackResponse, menuRef);
        });
      } else {
        processResults(tmdbResponse, menuRef);
      }
    }

    function processResults(response, menuRef) {
      if (response.total_results !== undefined) {
        if (response.total_results === 0) {
          // Ничего не добавляем, ждем hanime.tv
        } else {
        ...
        menuRef.push({
          title: `[${response.number_of_episodes ? 'TV' : 'MOVIE'}] ${response.title || response.name}`,
          card: response,
          source: 'tmdb'
        });
      }
    ...
    function displaySearchResults(menuItems) {
      if (menuItems.length === 0) {
        ...
        handleSelection(menuItems[0]);
      } else {
        Lampa.Noty.show('Найдено несколько результатов (' + menuItems.length + '), показываю меню выбора');
        Lampa.Select.show({
          title: 'Найти',
          items: menuItems,
          onBack: function() { Lampa.Controller.toggle('content'); },
          onSelect: function(item) { handleSelection(item); }
        });
      }
    }

    function handleSelection(item) {
      if (item.source === 'hanime') {
  ...
  function Card(data, userLang) {
    ...
    // Проверка доступности на hanime.tv
    API.searchHanime(data, function(hanimeResult) {
      if (hanimeResult) {
        item.find('.card__title').append(' <span style="color: #ff69b4">[H]</span>');
      }
    });
    ...
    this.headeraction = function () {
      ...
      $.ajax(settings).done(function (response) {
        var genreTranslations = {
          ...
          "Romance": "Романтика", "Samurai": "Самурайский", "School": "Школьный", "Sci-Fi": "Научная фантастика",
          "Seinen": "Сейнэн", "Shoujo": "Сёдзё", "Shoujo Ai": "Сёдзё-ай", "Shounen": "Сёнэн",
          "Shounen Ai": "Сёнэн-ай", "Slice of Life": "Повседневность", "Space": "Космос", "Sports": "Спорт",
          "Super Power": "Суперсила", "Supernatural": "Сверхъестественное", "Thriller": "Триллер", "Erotica": "Эротика",
      ...
      function mainMenu() {
        ...
        Lampa.Select.show({
          ...
          onBack: function() { Lampa.Controller.toggle("content"); },
          onSelect: function(a) {
            if (a.searchShikimori) search(); else submenu(a, mainMenu);
          }
        });
      }
