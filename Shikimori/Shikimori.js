(function () {
  ...
  // Вспомогательные функции для работы с объектами и генераторами (оставлены без изменений)
  ...
  // Улучшенная функция поиска с исправленной логикой сопоставления
  function search(animeData) {
    // Очистка названия от лишних слов (Season, Part) и лишних пробелов
    ...
    // Простая функция нечеткого соответствия (fuzzy matching) с использованием расстояния Левенштейна
    function fuzzyMatch(str1, str2) {
      const len1 = str1.length;
      const len2 = str2.length;
      if (len1 === 0 || len2 === 0) return 0;
      const dist = Array(len1 + 1).fill(null).map(() => Array(len2 + 1).fill(null));
      for (let i = 0; i <= len1; i++) dist[i][0] = i;
      for (let j = 0; j <= len2; j++) dist[0][j] = j;
      ...
      return 1 - dist[len1][len2] / Math.max(len1, len2);
    }

    // Попытка получить MAL ID через сторонний API
    $.get("https://arm.haglund.dev/api/v2/ids?source=myanimelist&id=" + animeData.id, function (response) {
      if (response && response.themoviedb) {
        // Если TMDB ID найден, напрямую получить данные
        getTmdb(response.themoviedb, animeData.kind, processResults);
      } else {
        // Если MAL ID отсутствует, выполнить поиск в TMDB с несколькими запросами
        searchTmdbWithMultipleQueries(animeData);
      }
    }).fail(function (jqXHR) {
      if (jqXHR.status === 404) {
        // Если запрос завершился ошибкой 404, выполнить поиск в TMDB
        searchTmdbWithMultipleQueries(animeData);
      } else {
    ...
    // Функция для поиска в TMDB с использованием нескольких полей
    function searchTmdbWithMultipleQueries(animeData) {
      const queries = [
        cleanName(animeData.name),
        cleanName(animeData.japanese),
        cleanName(animeData.english)
      ].filter(query => query); // Удаляем пустые запросы

      // Последовательный поиск по каждому запросу до первого совпадения
      function tryNextQuery(index) {
        if (index >= queries.length) {
          console.warn('Не найдено совпадений ни по одному запросу для:', animeData.name);
          Lampa.Noty.show('Не удалось найти совпадений в TMDB.');
          return;
        }
        searchTmdb(queries[index], function (tmdbResponse) {
    ...
    function searchTmdb(query, callback) {
      ...
      var request = "search/multi?api_key=" + apiKey + "&language=" + Lampa.Storage.field('language') + "&include_adult=true&query=" + encodeURIComponent(query);
      $.get(Lampa.Storage.field('proxy_tmdb') ? Lampa.Utils.protocol() + apiUrlProxy + request : apiUrlTMDB + request, callback);
    }

    // Функция для получения данных из TMDB по ID
    ...
    function handleTmdbResponse(tmdbResponse, animeData) {
      if (tmdbResponse.total_results === 0) {
        console.warn('Не найдено результатов для запроса:', tmdbResponse.query);
      } else if (tmdbResponse.total_results === 1) {
        processResults(tmdbResponse.results[0]);
      } else {
        // Если найдено несколько результатов, выбрать лучший вариант
        const bestMatch = selectBestMatch(tmdbResponse.results, animeData);
        if (bestMatch) {
          processResults(bestMatch);
        } else {
          // Если лучший вариант не найден, предложить пользователю выбор
          presentOptionsToUser(tmdbResponse.results);
        }
      }
    }

    // Выбор лучшего совпадения на основе нечеткого соответствия
    function selectBestMatch(results, animeData) {
      let bestScore = 0;
      let bestMatch = null;
      results.forEach(result => {
        const title = result.title || result.name;
        const score = fuzzyMatch(cleanName(animeData.name), cleanName(title));
        if (score > bestScore) {
          bestScore = score;
          bestMatch = result;
        }
      });
      // Порог для автоматического выбора (например, 80%)
      if (bestScore >= 0.8) {
        console.log('Автоматически выбран лучший вариант:', bestMatch.title || bestMatch.name, 'с рейтингом:', bestScore);
        return bestMatch;
      }
      return null;
    }

    // Предложение пользователю выбрать из списка
    function presentOptionsToUser(results) {
      var menu = results.map(function (item) {
        return {
          title: "[" + item.media_type.toUpperCase() + "] " + (item.name ? item.name : item.title),
          card: item
        };
      });
      Lampa.Select.show({
        title: 'Выберите правильное совпадение',
        items: menu,
        onBack: function () {
          Lampa.Controller.toggle("content");
        },
        onSelect: function (a) {
          processResults(a.card);
        }
      });
    }

    // Обработка выбранного результата
    function processResults(card) {
      Lampa.Activity.push({
        url: '',
        component: 'full',
        id: card.id,
        method: card.media_type,
        card: card
      });
    }
  }
  ...
  // Класс для создания карточки аниме (без изменений)
  ...
  // Основной компонент для отображения каталога (без изменений)
  ...
  // Компонент для расширения информации в карточке (без изменений)
  function Component() {
  ...
  // Добавление кнопки в меню (без изменений)
  function add() {
