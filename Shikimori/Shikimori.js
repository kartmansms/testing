(function () {
    ...
    function _regeneratorRuntime() {
        _regeneratorRuntime = function () { return e; };
        var t, e = {}, r = Object.prototype, n = r.hasOwnProperty,
        ...
        function wrap(t, e, r, n) {
            var i = e && e.prototype instanceof Generator ? e : Generator,
                a = Object.create(i.prototype), c = new Context(n || []);
            return o(a, "_invoke", { value: makeInvokeMethod(t, r, c) }), a;
        }
        function tryCatch(t, e, r) {
            try { return { type: "normal", arg: t.call(e, r) }; } catch (t) { return { type: "throw", arg: t }; }
        }
        e.wrap = wrap;
        ...
        function defineIteratorMethods(t) {
            ["next", "throw", "return"].forEach(function (e) {
                define(t, e, function (t) { return this._invoke(e, t); });
            });
        }
            ...
            function invoke(r, o, i, a) {
                ...
                if ("throw" !== c.type) {
                    ...
                    return h && "object" == typeof h && n.call(h, "__await") ? e.resolve(h.__await).then(function (t) {
                        ...
                        invoke("throw", t, i, a);
                    }) : e.resolve(h).then(function (t) { u.value = t, i(u); }, function (t) { return invoke("throw", t, i, a); });
                }
                a(c.arg);
            }
            var r;
            o(this, "_invoke", {
                value: function (t, n) {
                    function callInvokeWithMethodAndArg() {
                        return new e(function (e, r) { invoke(t, n, e, r); });
                    }
    ...
    function _nonIterableSpread() { throw new TypeError("Неверная попытка распространить неитерируемый экземпляр.\nДля того чтобы быть итерируемым, не-массивные объекты должны иметь метод [Symbol.iterator]()."); }

    // Основная функция для выполнения GraphQL-запроса к Shikimori API
    function main(params, oncomplete, onerror) {
        $(document).ready(function () {
            function safeValue(value) { return String(value || '').replace(/["\\]/g, ''); }
            var query = "\n    query Animes {\n    animes(limit: 36, order: ".concat(safeValue(params.sort) || 'aired_on', ", page: ").concat(parseInt(params.page, 10) || 1, "\n    ");
            if (params.kind) query += ", kind: \"".concat(safeValue(params.kind), "\"");
            if (params.status) query += ", status: \"".concat(safeValue(params.status), "\"");
            if (params.genre) query += ", genre: \"".concat(safeValue(params.genre), "\"");
            if (params.seasons) query += ", season: \"".concat(safeValue(params.seasons), "\"");
            query += ") {\n                    id\n                    name\n                    russian\n                    licenseNameRu\n                    english\n                    japanese\n                    kind\n                    score\n                    status\n                    season\n                    airedOn { year }\n                    poster {\n                        originalUrl\n                    }\n                }\n            }\n        ";
            $.ajax({
                ...
                success: function (response) {
                    if (response && response.data && response.data.animes) oncomplete(response.data.animes);
                    else {
                        console.error('Некорректный ответ Shikimori:', response);
                        onerror(response);
                    }
                },
                error: function (error) { console.error('Ошибка:', error); onerror(error); }
            });
        });
    }
    ...
    function search(animeData) {
        function cleanName(name) {
            ...
            var cleanedName = String(name || '').replace(regex, '').trim();
            cleanedName = cleanedName.replace(/\s{2,}/g, ' ');
            return cleanedName;
        }

        function tmdbType(kind) { return kind === 'movie' ? 'movie' : 'tv'; }

        function findByName() {
            searchTmdb(animeData.name || animeData.russian || animeData.japanese, function (tmdbResponse) {
                handleTmdbResponse(tmdbResponse, animeData.japanese || animeData.russian || animeData.name);
            });
        }

        $.get("https://arm.haglund.dev/api/v2/ids?source=myanimelist&id=".concat(animeData.id), function (response) {
            if (response === null || response.themoviedb === null || response.themoviedb === undefined) {
                findByName();
            } else {
                getTmdb(response.themoviedb, tmdbType(animeData.kind), function (tmdbResponse) {
                    if (tmdbResponse) processResults(tmdbResponse);
                    else findByName();
                });
            }
        }).fail(function (jqXHR) {
            if (jqXHR.status === 404) {
                ...
                console.error('Ошибка при получении данных с animeapi:', jqXHR.status);
                findByName();
            }
        });
        ...
        function searchTmdb(query, callback) {
            ...
            $.get(Lampa.Storage.field('proxy_tmdb') ? Lampa.Utils.protocol() + apiUrlProxy + request : apiUrlTMDB + request, callback)
                .fail(function (error) { console.error('Ошибка TMDB search:', error); callback({ total_results: 0 }); });
        }
        ...
        function getTmdb(id, type, callback) {
            ...
            $.get(Lampa.Storage.field('proxy_tmdb') ? Lampa.Utils.protocol() + apiUrlProxy + request : apiUrlTMDB + request, callback)
                .fail(function (error) { console.error('Ошибка TMDB get:', error); callback(null); });
        }
        ...
        function handleFallbackResponse(fallbackResponse) { processResults(fallbackResponse); }

        function processResults(response) {
            var menu = [];
            if (!response) return;
            if (response.total_results !== undefined) {
                var results = (response.results || []).filter(function (item) { return item && (item.media_type === 'movie' || item.media_type === 'tv'); });
                if (!results.length) {
                    Lampa.Noty.show('Не удалось найти данные');
                } else if (results.length === 1) {
                    Lampa.Activity.push({
                        url: '',
                        component: 'full',
                        id: results[0].id,
                        method: results[0].media_type,
                        card: results[0]
                    });
                } else {
                    results.forEach(function (animeItem) {
                        menu.push({
                            title: "[".concat(String(animeItem.media_type || '').toUpperCase(), "] ").concat(animeItem.name ? animeItem.name : animeItem.title),
                            card: animeItem
                        });
                    });
                    Lampa.Select.show({
    ...
    function Card(data, userLang) {
        var typeTranslations = {
            ...
            'special': 'Спешл', 'tv_special': 'ТВ Спешл', 'music': 'Музыка', 'pv': 'PV', 'cm': 'CM'
        };
        var statusTranslations = { 'anons': 'Анонс', 'ongoing': 'Онгоинг', 'released': 'Вышло' };
        var formattedSeason = data.season ? data.season.replace(/_/g, ' ')
            .replace(/^\w/, function (c) { return c.toUpperCase(); })
            .replace(/(winter|spring|summer|fall)/gi, function (match) {
                return { 'winter': 'Зима', 'spring': 'Весна', 'summer': 'Лето', 'fall': 'Осень' }[match.toLowerCase()];
            }) : '';
        ...
        // ВАЖНО: добавлен tabindex="0" и role="button", чтобы карточка была доступна для фокуса с пульта
        var kind = data.kind || '';
        var poster = data.poster && data.poster.originalUrl ? data.poster.originalUrl : './img/img_broken.svg';
        var title = userLang === 'ru' ? data.russian || data.name || data.japanese : data.name || data.japanese || data.russian;
        var item = $('<div class="Shikimori card selector layer--visible layer--render" tabindex="0" role="button">' +
            '<div class="Shikimori card__view">' +
            '<img src="' + escapeHtml(poster) + '" class="Shikimori card__img" />' +
            '<div class="Shikimori card__type">' + escapeHtml(typeTranslations[kind] || kind.toUpperCase()) + '</div>' +
            '<div class="Shikimori card__vote">' + escapeHtml(data.score || '0') + '</div>' +
            '<div class="Shikimori card__season">' + escapeHtml(formattedSeason) + '</div>' +
            '<div class="Shikimori card__status">' + escapeHtml(statusTranslations[data.status] || capitalizeFirstLetter(data.status)) + '</div>' +
            '</div>' +
            '<div class="Shikimori card__title">' + escapeHtml(title || 'Без названия') + '</div>' +
            '</div>');

        this.data = data;
        this.element = item;
        this.open = function () { API.search(data); };
    ...
    function Component$1(object) {
        ...
        // Убран mask: true – на Tizen маска может перехватывать фокус
        ...
        function openAnime(item, event) {
            if (!item || enterLocked) return false;
            enterLocked = true;
            if (event) {
            ...
            item.open();
            setTimeout(function () { enterLocked = false; }, 600);
            return false;
        }
        ...
        this.build = function (result) {
            ...
            if (result.length < 36) noMore = true;
            scroll.minus();
            ...
            scroll.onEnd = function () {
                ...
                loadingNext = true;
                object.page++;
                API.main(object, _this.build.bind(_this), function () { loadingNext = false; });
            };
        ...
        this.headeraction = function () {
            var settings = { "url": "https://shikimori.one/api/genres", "method": "GET", "timeout": 0 };
            var filters = {};
            var currentYear = new Date().getFullYear();
            filters.kind = { title: 'Жанр', items: [{ title: 'Жанры загружаются...', disabled: true }] };

            $.ajax(settings).done(function (response) {
                var genreTranslations = {
                    "Action": "Экшен", "Adventure": "Приключения", "Cars": "Машины", "Comedy": "Комедия",
                    "Dementia": "Деменция", "Demons": "Демоны", "Drama": "Драма", "Ecchi": "Этти",
                    "Fantasy": "Фэнтези", "Game": "Игра", "Harem": "Гарем", "Historical": "Исторический",
                    "Horror": "Ужасы", "Josei": "Дзёсей", "Kids": "Детский", "Magic": "Магия",
                    "Martial Arts": "Боевые искусства", "Mecha": "Меха", "Military": "Военный", "Music": "Музыка",
                    "Mystery": "Мистика", "Parody": "Пародия", "Police": "Полиция", "Psychological": "Психологический",
                    "Romance": "Романтика", "Samurai": "Самурайский", "School": "Школьный",
                    "Sci-Fi": "Научная фантастика", "Seinen": "Сейнэн", "Shoujo": "Сёдзё", "Shoujo Ai": "Сёдзё-ай",
                    "Shounen": "Сёнэн", "Shounen Ai": "Сёнэн-ай", "Slice of Life": "Повседневность",
                    "Space": "Космос", "Sports": "Спорт", "Super Power": "Суперсила", "Supernatural": "Сверхъестественное",
                    "Thriller": "Триллер", "Erotica": "Эротика", "Hentai": "Хентай", "Yaoi": "Яой",
                    "Yuri": "Юри", "Gourmet": "Гурман", "Work Life": "Трудяги", "Vampire": "Вампиры"
                };
                var filteredResponse = response.filter(function (item) { return item.entry_type === "Anime"; }).map(function (item) {
                    return _objectSpread2(_objectSpread2({}, item), {}, { title: genreTranslations[item.name] || item.name, name: undefined });
                });
                filters.kind = { title: 'Жанр', items: filteredResponse };
            }).fail(function (error) {
                console.error('Ошибка загрузки жанров:', error);
                filters.kind = { title: 'Жанр', items: [{ title: 'Жанры недоступны', disabled: true }] };
            });
            ...
            filters.AnimeKindEnum = {
                ...
                items: [
                    ...
                    { title: "Спешл", code: "special" }, { title: "ТВ Спешл", code: "tv_special" },
            ...
            function generateDynamicSeasons() {
                ...
                var seasonTranslations = { 'winter': 'Зима', 'spring': 'Весна', 'summer': 'Лето', 'fall': 'Осень' };
                ...
                } else if (currentMonth >= 2 && currentMonth <= 4) currentSeasonIndex = 1;
                else if (currentMonth >= 5 && currentMonth <= 7) currentSeasonIndex = 2;
                else if (currentMonth >= 8 && currentMonth <= 10) currentSeasonIndex = 3;
                for (var i = 0; i < 4; i++) {
                    var seasonIndex = (currentSeasonIndex - i + 4) % 4;
                    var seasonYear = currentSeasonYear;
                    if (i > 0 && currentSeasonIndex - i < 0) seasonYear = currentSeasonYear - 1;
                    if (i > currentSeasonIndex) seasonYear = currentSeasonYear - 1;
                    if (seasonIndex === 0 && currentMonth === 11 && i > 0) seasonYear = currentSeasonYear - 1;
                    seasons.push({
                        code: seasonsOrder[seasonIndex] + '_' + seasonYear,
                        title: seasonTranslations[seasonsOrder[seasonIndex]] + ' ' + seasonYear
                    });
                }
                return seasons;
            }
            ...
            function generateYearRanges() {
                var ranges = [];
                for (var year = currentYear; year >= currentYear - 4; year--) ranges.push({ code: '' + year, title: year + ' год' });
                for (var startYear = currentYear; startYear >= currentYear - 20; startYear -= 5) {
                    var endYear = startYear - 4;
                    if (endYear <= startYear) ranges.push({ code: endYear + '_' + startYear, title: endYear + '–' + startYear });
                }
                return ranges;
            }

            function generateSeasonJSON() { return generateDynamicSeasons().concat(generateYearRanges()); }

            filters.seasons = { title: 'Сезон', items: generateSeasonJSON() };

            var serverElement = head.find('.Shikimori__search');
            function queryForShikimori() {
                var query = {};
                filters.AnimeKindEnum.items.forEach(function (a) { if (a.selected) query.kind = a.code; });
                filters.status.items.forEach(function (a) { if (a.selected) query.status = a.code; });
                if (filters.kind && filters.kind.items) filters.kind.items.forEach(function (a) { if (a.selected && !a.disabled) query.genre = a.id; });
            ...
            function selected(where) {
                if (!where || !where.items) return;
                var title = [];
                where.items.forEach(function (a) { if (a.selected || a.checked) title.push(a.title); });
            ...
            function submenu(item, main) {
                ...
                Lampa.Select.show({
                    ...
                    items: item.items,
                    onBack: main,
                    onSelect: function (a) { select(item.items, a); main(); }
                });
            }
            ...
            function mainMenu() {
                ...
                Lampa.Select.show({
                    title: 'Фильтры',
                    items: [{ title: Lampa.Lang.translate('search_start'), searchShikimori: true }, filters.status, filters.AnimeKindEnum, filters.kind, filters.sort, filters.seasons],
                    onBack: function () { Lampa.Controller.toggle("content"); },
                    onSelect: function (a) { if (a.searchShikimori) searchAction(); else submenu(a, mainMenu); }
                });
            }

            function searchAction() {
                var query = queryForShikimori();
                var params = { url: '', title: 'Shikimori', component: 'Shikimori', page: 1 };
        ...
        this.body = function (data) {
            ...
            data.forEach(function (anime) {
                ...
                // События фокуса и активации с пульта
                cardElement.on("hover:focus", function () {
                    last = cardElement[0];
                    active = items.indexOf(item);
                    scroll.update(cardElement, true);
                });
                // Обработчик активации (Enter на пульте)
                cardElement.on("hover:enter click tap mouseup", function (e) {
                    return openAnime(item, e);
                });
                // На части Samsung/Tizen Enter приходит как keyup и не превращается в hover:enter.
                cardElement.on('keydown keyup', function (e) {
            ...
            // Автоматически фокусируем первую карточку после отрисовки
            if (items.length > 0 && !last) {
                var firstCard = items[0].render(true);
        ...
        this.start = function () {
            ...
            Lampa.Controller.add("content", {
                ...
                enter: function enter() {
                    if (active > -1 && items[active]) return openAnime(items[active]);
                    if (last) return openAnime(items.filter(function (item) { return item.element && item.element[0] === last; })[0]);
                },
        ...
        Lampa.Listener.follow("full", _asyncToGenerator(_regeneratorRuntime().mark(function _callee(e) {
            ...
            return _regeneratorRuntime().wrap(function _callee$(_context) {
                while (1) switch (_context.prev = _context.next) {
                    case 0:
                        if (!(e.type === "complite")) { _context.next = 21; break; }
                        _context.prev = 1;
                        _context.next = 4;
                        return $.ajax({ url: "https://arm.haglund.dev/api/v2/themoviedb?id=".concat(e.object.id), method: "GET", timeout: 0 });
                    case 4:
                        getMAL = _context.sent;
                        if (getMAL.length) { _context.next = 8; break; }
                        console.warn("Данные для предоставленного ID не найдены.");
                        return _context.abrupt("return");
                    case 8:
                        _context.next = 10;
                        return $.ajax({ url: "https://shikimori.one/api/animes/".concat(getMAL[0].myanimelist), method: "GET", timeout: 0 });
                    case 10:
                        response = _context.sent;
                        dubbers = response.fandubbers && response.fandubbers.length ? "\n                    <div class=\"full-descr__info\">\n                        <div class=\"full-descr__info-name\">Фандабберы</div>\n                        <div class=\"full-descr__text\">".concat(escapeHtml(response.fandubbers.join(', ')), "</div>\n                    </div>") : '';
                        subbers = response.fansubbers && response.fansubbers.length ? "\n                    <div class=\"full-descr__info\">\n                        <div class=\"full-descr__info-name\">Фансабберы</div>\n                        <div class=\"full-descr__text\">".concat(escapeHtml(response.fansubbers.join(', ')), "</div>\n                    </div>") : '';
                        e.object.activity.render().find(".full-descr__right").append(dubbers, subbers);
                        shikimoriRates = "<div class=\"full-start__rate rate--shikimori\"><div>".concat(escapeHtml(response.score || '0'), "</div><div>Shikimori</div></div>");
                        if (!e.object.activity.render().find(".rate--shikimori").length) e.object.activity.render().find(".full-start-new__rate-line").prepend(shikimoriRates);
                        _context.next = 21;
                        break;
                    case 18:
                        _context.prev = 18;
