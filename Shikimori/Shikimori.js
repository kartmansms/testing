(function () {
    ...
    this.headeraction = function () {
      ...
      $.ajax(settings).done(function (response) {
        ...
        var filteredResponse = response.filter(function (item) {
          ...
          return Object.assign({}, item, { title: genreTranslations[item.name] || item.name, name: undefined });
        });
      ...
      filters.sort = {
        title: 'Сортировка',
        items: [
          { title: "По рейтингу", code: "ranked" }, { title: "По популярности", code: "popularity" }, { title: "По алфавиту", code: "name" },
          { title: "По дате выхода", code: "aired_on" }, { title: "По типу", code: "kind" }, { title: "По количеству эпизодов", code: "episodes" },
  ...
  function add() {
    var button = $("<li class=\"menu__item selector\">\n" +
                   "    <div class=\"menu__ico\">\n" +
                   "        <img src=\"https://kartmansms.github.io/testing/icons/Shiki_icon.svg\" class=\"shikimori-icon\" style=\"filter: brightness(0) invert(1);\" />\n" + // Белая иконка по умолчанию
                   "    </div>\n" +
    ...
    // Слушатель для изменения цвета иконки при смене активной вкладки
    Lampa.Listener.follow('activity', function (e) {
      var shikimoriIcon = $('.shikimori-icon');
      if (e.activity && e.activity.component === 'Shikimori') {
        shikimoriIcon.css('filter', 'brightness(0)'); // Чёрная иконка при активной вкладке
      } else {
        shikimoriIcon.css('filter', 'brightness(0) invert(1)'); // Белая иконка для неактивной вкладки
      }
    });
  }
