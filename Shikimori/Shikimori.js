  ...
  function main(params, oncomplite, onerror) {
    $(document).ready(function () {
      ...
      if (params.isTop100) {
        ...
        // Обычный запрос для одной страницы
        $.ajax({
          url: 'https://shikimori.one/api/graphql',
          method: 'POST',
          contentType: 'application/json',
...
>>>>>>> fdeec19de66a32669e465954aa4369edcc809ff0
    ...
    // Основная функция для выполнения GraphQL-запроса к Shikimori API (модифицирована для поддержки "Топ100")
    function main(params, oncomplite, onerror) {
        $(document).ready(function () {
            // Устанавливаем limit: 50 для "Топ100", иначе дефолтное значение 36
            var limit = params.isTop100 ? 50 : (params.limit || 36);
            var query = "\n	query Animes {\n	animes(limit: ".concat(limit, ", order: ").concat(params.sort || 'aired_on', ", page: ").concat(params.page, "\n	");
            ...
            // Если это запрос для "Топ100", делаем два запроса (page 1 и page 2) и объединяем результаты
            if (params.isTop100) {
                var requests = [
                    $.ajax({
                        url: 'https://shikimori.one/api/graphql',
    ...
    // Функция для начала процесса авторизации
    function startAuth() {
        ...
        Lampa.Noty.show('Авторизация с Shikimori...');
        window.location.href = authUrl;

        // После перенаправления обратно на redirect_uri нужно обработать code и получить access_token
        // Это требует серверной части, так как обмен кода на токен должен быть защищенным
        // Здесь предполагается, что access_token будет сохранен в localStorage после успешной авторизации
    }

    // Функция для получения списка аниме по статусу
    function showAnimeList(status) {
        var accessToken = localStorage.getItem('shikimori_access_token');
        if (!accessToken) {
            Lampa.Noty.show('Пожалуйста, авторизуйтесь сначала.');
            return;
        }

        // Запрос к API Shikimori для получения списка аниме пользователя
        $.ajax({
            ...
            success: function (response) {
                // Преобразуем user_rates в формат, подходящий для отображения
                var animeList = response.map(function (rate) {
                    return rate.anime;
                });
                displayAnimeList(animeList, status);
            },
            error: function (error) {
                console.error('Ошибка при получении списка аниме:', error);
                Lampa.Noty.show('Не удалось получить список аниме.');
            }
        });
    }

    // Функция для отображения списка аниме
    function displayAnimeList(data, status) {
        var listTitle = {
    ...
    function showProfileMenu() {
        var menuItems = [
            { title: 'Авторизация', action: 'auth' },
            { title: 'Запланировано', action: 'planned' },
            { title: 'Смотрю', action: 'watching' },
            { title: 'Просмотрено', action: 'completed' },
            { title: 'Брошено', action: 'dropped' }
        ];
        ...
        Lampa.Select.show({
            ...
            onSelect: function (item) {
                if (item.action === 'auth') {
                    ...
                    showAnimeList(item.action);
                }
            }
        });
    }

    // Основной компонент для отображения каталога (модифицирован для поддержки "Топ100" и профиля)
    function Component$1(object) {
        var userLang = Lampa.Storage.field('language');
        var network = new Lampa.Reguest();
        var scroll = new Lampa.Scroll({
        ...
        var items = [];
        var html = $("<div class='Shikimori-module'></div>");
        // Добавляем кнопку "Профиль" рядом с "Фильтр"
        var head = $("<div class='Shikimori-head torrent-filter'><div class='Shikimori__home simple-button simple-button--filter selector'>Главная</div><div class='Shikimori__top100 simple-button simple-button--filter selector'>Топ100</div><div class='Shikimori__search simple-button simple-button--filter selector'>Фильтр</div><div class='Shikimori__profile simple-button simple-button--filter selector'>Профиль</div></div>");
        var body = $('<div class="Shikimori-catalog--list category-full"></div>');
        var active, last;
        ...
        this.build = function (result) {
            ...
            // Отключаем пагинацию для "Топ100"
            if (!object.isTop100) {
                scroll.onEnd = function () {
                    object.page++;
                    API.main(object, _this.build.bind(_this), _this.empty.bind(_this));
        ...
        this.headeraction = function () {
            ...
            var serverElement = head.find('.Shikimori__search');
            var profileElement = head.find('.Shikimori__profile');
            function queryForShikimori() {
                var query = {};
                filters.AnimeKindEnum.items.forEach(function (a) {
                    if (a.selected) query.kind = a.code;
            ...
            profileElement.on('hover:enter', function () {
                showProfileMenu();
            });
            var homeElement = head.find('.Shikimori__home');
            homeElement.on('hover:enter', function () {
    ...
    // Новый компонент для отображения списков профиля
    function ShikimoriListComponent(object) {
        var userLang = Lampa.Storage.field('language');
        var network = new Lampa.Reguest();
        var scroll = new Lampa.Scroll({
            mask: true,
        ...
        this.build = function (result) {
            ...
            Component$1.prototype.headeraction.call(this);
            this.body(result);
            scroll.append(head);
            scroll.append(body);
            html.append(scroll.render(true));
        ...
        this.start = Component$1.prototype.start;
        this.pause = Component$1.prototype.pause;
        this.stop = Component$1.prototype.stop;
        this.render = Component$1.prototype.render;
        this.destroy = Component$1.prototype.destroy;
    }
    ...
    function startPlugin() {
        ...
        Lampa.Component.add('ShikimoriList', ShikimoriListComponent);
        Component();
