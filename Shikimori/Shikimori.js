(function () {
  ...
  function translateSeason(season) {
    ...
    const seasonName = seasonTranslations[parts[0]] || parts[0];
    const year = parts[1] || '';
  ...
  function Card(data, userLang) {
    ...
    var item = Lampa.Template.get("Shikimori-Card", {
      ...
      type: translateType(data.kind.toUpperCase()),
      status: capitalizeFirstLetter(translateStatus(data.status)),
      rate: data.score,
      title: userLang === 'ru' ? data.russian || data.name || data.japanese : data.name || data.japanese,
      season: formattedSeason || data.airedOn.year // Проверка на null
    });
