(function () {
        ...
        $(document).ready(function () {
            ...
            $.ajax({
                ...
                success: function success(response) {
                    console.log('GraphQL запрос успешен:', response);
                    oncomplite(response.data.animes);
                },
                error: function error(_error) {
                    console.error('Ошибка GraphQL-запроса:', _error);
                    onerror(_error);
                }
            });
        });
    ...
    function search(animeData) {
        ...
        $.get("https://arm.haglund.dev/api/v2/ids?source=myanimelist&id=".concat(animeData.id), function (response) {
            console.log('Запрос к animeapi.my.id успешен:', response);
            if (response === null) {
                console.log('Шаг #1: Данные не найдены, ищем на TMDB по имени:', animeData.name);
                ...
                console.log('Шаг #2: TMDB ID отсутствует, ищем на TMDB по имени:', animeData.name);
                searchTmdb(animeData.name, function (tmdbResponse) {
                    handleTmdbResponse(tmdbResponse, animeData.japanese);
                });
            } else {
                console.log('Шаг #3: Найден TMDB ID:', response.themoviedb, 'Тип:', animeData.kind);
                getTmdb(response.themoviedb, animeData.kind, processResults);
            }
        }).fail(function (jqXHR) {
            console.error('Ошибка запроса к animeapi.my.id:', jqXHR.status, jqXHR.responseText);
            if (jqXHR.status === 404) {
                console.log('404: ID не найден, ищем на TMDB по имени:', animeData.name);
                searchTmdb(animeData.name, function (tmdbResponse) {
                    handleTmdbResponse(tmdbResponse, animeData.japanese);
                });
            }
        ...
        function searchTmdb(query, callback) {
            ...
            $.get(Lampa.Storage.field('proxy_tmdb') ? Lampa.Utils.protocol() + apiUrlProxy + request : apiUrlTMDB + request, function (response) {
                console.log('Запрос к TMDB успешен:', response);
                callback(response);
            }).fail(function (jqXHR) {
                console.error('Ошибка запроса к TMDB:', jqXHR.status, jqXHR.responseText);
        ...
        function getTmdb(id, type = 'movie', callback) {
            ...
            $.get(Lampa.Storage.field('proxy_tmdb') ? Lampa.Utils.protocol() + apiUrlProxy + request : apiUrlTMDB + request, function (response) {
                console.log('Запрос к TMDB по ID успешен:', response);
                callback(response);
            }).fail(function (jqXHR) {
                console.error('Ошибка запроса к TMDB по ID:', id, jqXHR.status, jqXHR.responseText);
                callback(null);
            });
        }
        ...
        function handleTmdbResponse(tmdbResponse, fallbackQuery) {
            if (tmdbResponse.total_results === 0) {
                console.log('TMDB: Результатов не найдено, пробуем запасной запрос:', fallbackQuery);
        ...
        function handleFallbackResponse(fallbackResponse) {
            console.log('Запасной запрос к TMDB:', fallbackResponse);
            processResults(fallbackResponse);
        }

        function processResults(response) {
            var menu = [];
            if (response && response.total_results !== undefined) {
                console.log('Обработка результатов поиска TMDB:', response);
                if (response.total_results === 0) {
                    Lampa.Noty.show('Не смог победить!!!');
                    console.log('TMDB: Ничего не найдено');
                } else if (response.total_results === 1) {
                    console.log('TMDB: Найден один результат:', response.results[0]);
                    pushActivity(response.results[0]);
                } else if (response.total_results > 1) {
                    console.log('TMDB: Найдено несколько результатов:', response.results);
                    ...
                    Lampa.Select.show({
                        ...
                        onSelect: function onSelect(a) {
                            console.log('Выбран вариант:', a.card);
                            pushActivity(a.card);
                        }
                    });
                }
            } else if (response && response.id) {
                console.log('TMDB: Найден один элемент по ID:', response);
                pushActivity(response);
            } else {
                console.error('TMDB: Неверный формат ответа или ошибка:', response);
                Lampa.Noty.show('Произошла ошибка при обработке данных');
            }
        }

        function pushActivity(card) {
            var mediaType = card.number_of_episodes ? 'tv' : 'movie';
            console.log('Открытие карточки:', { id: card.id, type: mediaType });
            Lampa.Activity.push({
                url: '',
                component: 'full',
                id: card.id,
    ...
    function Component$1(object) {
        ...
        this.headeraction = function () {
            ...
            $.ajax(settings).done(function (response) {
                console.log('Запрос жанров успешен:', response);
                ...
                console.error('Ошибка запроса жанров:', jqXHR.status, jqXHR.responseText);
            });
