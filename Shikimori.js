(function () {
    ...
    function search(animeData) {
        ...
        // Первый GET запрос к https://arm.haglund.dev/api/v2/ids
        $.get("https://arm.haglund.dev/api/v2/ids?source=myanimelist&id=".concat(animeData.id), function (response) {
            if (response === null) {
                ...
                // Если получили null, продолжаем искать на TMDB
                ...
                // Если themoviedb: null, делаем запрос к TMDB
                searchTmdb(animeData.name, function (tmdbResponse) {
                    handleTmdbResponse(tmdbResponse, animeData.japanese);
                });
            } else {
                console.log('Мы здесь шаг#3', animeData.kind);
                // Если themoviedb не равно null, делаем запрос к TMDB
        ...
        function searchTmdb(query, callback) {
            ...
            $.get(Lampa.Storage.field('proxy_tmdb') ? Lampa.Utils.protocol() + apiUrlProxy + request : apiUrlTMDB + request, callback)
                .fail(function (error) {
                    console.error('Ошибка TMDB search:', error);
        ...
        function getTmdb(id, type = 'movie', callback) {
            ...
            $.get(Lampa.Storage.field('proxy_tmdb') ? Lampa.Utils.protocol() + apiUrlProxy + request : apiUrlTMDB + request, callback)
                .fail(function (error) {
                    console.error('Ошибка TMDB get:', error);
                    callback(null);
                });
        }
    ...
    // Определяем индекс текущего сезона на основе месяца
    ...
    // Функция для получения года предыдущего сезона
    function getPrevYear(seasonIdx, year, month) {
        if (seasonIdx === 0) { // текущий сезон - зима
            if (month === 11) { // декабрь -> зима следующего года, предыдущий - осень текущего
                return year;
            } else { // январь/февраль -> зима текущего года, предыдущий - осень прошлого
                return year - 1;
            }
        } else {
            // для весны, лета, осени предыдущий сезон в том же году или предыдущем при переходе
            // через зиму (осень -> зима)
            // Текущий сезон не зима, значит предыдущий не уходит в прошлый календарный год
            // Нет, может уйти. Лето 2025 -> Весна 2025. Осень 2025 -> Лето 2025. Зима 2026 (дек) -> Осень 2026.
            // Весна 2025 -> Зима 2025 (янв/фев) или Зима 2024 (дек2023)? month важен.
            // Если текущий сезон весна (1) и месяц март (2), то предыдущий сезон - зима 2024 (декабрь 2023 - февраль 2024).
            // Индекс предыдущего: (1 - 1 + 4) % 4 = 0 (зима). Год: если индекс текущего 0 (зима) и мес не 11, то год-1. Нет.
            // Если индекс текущего 1 (весна), месяц 2 (март), предыдущий индекс 0 (зима). Это зима 2024 (дек2023/янв2024/фев2024). Год 2024.
            // Если индекс текущего 2 (лето), месяц 5 (июнь), предыдущий индекс 1 (весна). Это весна 2025. Год 2025.
            // Если индекс текущего 3 (осень), месяц 8 (сентябрь), предыдущий индекс 2 (лето). Это лето 2025. Год 2025.
            // Если индекс текущего 0 (зима), мес 11 (дек), индекс пред 3 (осень). Это осень текущего года (2025). Год 2025.
            // Если индекс текущего 0 (зима), мес 0 (янв), индекс пред 3 (осень). Это осень предыдущего года (2024). Год 2024.
            // Общее правило:
            // prevIdx = (currentSeasonIndex - 1 + 4) % 4;
            // Если текущий сезон зима (0) и месяц НЕ декабрь (11), то prevYear = currentSeasonYear - 1.
            // Иначе prevYear = currentSeasonYear.
            // prevIdx = 3 (осень). prevYear = currentSeasonYear - 1 = 2025 - 1 = 2024. Нет!
            // Если зима (0) и мес 11 (дек), то prevYear = currentSeasonYear = 2025. prevIdx=3. Осень 2025. ОК.
            // Если зима (0) и мес 0 (янв), то prevYear = currentSeasonYear - 1 = 2025 - 1 = 2024. prevIdx=3. Осень 2024. ОК.
            // Если лето (2) и мес 5 (июнь), prevIdx=1 (весна). prevYear = currentSeasonYear = 2025. Весна 2025. ОК.
            // Если весна (1) и мес 2 (март), prevIdx=0 (зима). prevYear = ?
            // Если мес 2 (март), это зима 2025? Нет. Это весна 2025. currentSeasonYear = 2025. currentMonth = 2.
            // currentSeasonIndex = 1 (весна). prevIdx = 0 (зима). prevYear = currentSeasonYear = 2025? Нет. Зима 2025 была в мес 0,1,11.
            // Весна 2025 (март) -> предыдущая зима была в конце 2024 (декабрь 2024). prevYear = 2024.
            // Значит, если prevIdx (зима) == 0 и текущий индекс (весна) == 1, то prevYear = currentSeasonYear - 1.
            // Или, если (currentSeasonIndex - 1 + 4) % 4 == 0 (пред. зима) и currentSeasonIndex == 1 (тек. весна), то prevYear = currentSeasonYear - 1.
            // Или, если currentSeasonIndex == 1 (весна), то prevIdx = 0 (зима), prevYear = currentSeasonYear - 1.
            // Или, если currentSeasonIndex == 0 (зима) и мес 11 (дек), prevYear = currentSeasonYear.
            // Или, если currentSeasonIndex == 0 (зима) и мес != 11 (не дек), prevYear = currentSeasonYear - 1.
            // Это то же, что было раньше.
            var prevIdx = (currentSeasonIndex - 1 + 4) % 4;
            var year = currentSeasonYear;
            if (currentSeasonIndex === 0 && currentMonth !== 11) {
                year = currentSeasonYear - 1;
            }
            // Если предыдущий сезон - зима и текущий - весна, год сдвигающийся
            // Но это частный случай формулы выше.
            // if (prevIdx === 0 && currentSeasonIndex === 1) {
            //     year = currentSeasonYear - 1;
            // }
            return year;
        }
    ...
    // Индекс предыдущего сезона
    var prevSeasonIndex1 = (currentSeasonIndex - 1 + 4) % 4;
    var prevYear1 = getPrevYear(currentSeasonIndex, currentYear, currentMonth);

    var prevSeasonIndex2 = (prevSeasonIndex1 - 1 + 4) % 4;
    var prevYear2;
    if (prevSeasonIndex1 === 0 && (currentSeasonIndex === 1 || (currentSeasonIndex === 0 && currentMonth !== 11))) { // если prev1 - зима
        prevYear2 = prevYear1 - 1;
    } else {
        prevYear2 = prevYear1;
    }

    var prevSeasonIndex3 = (prevSeasonIndex2 - 1 + 4) % 4;
    var prevYear3;
    if (prevSeasonIndex2 === 0 && prevSeasonIndex1 === 1) { // если prev2 - зима, prev1 - весна
        prevYear3 = prevYear2 - 1;
    } else if (prevSeasonIndex1 === 0 && currentSeasonIndex === 1) { // если prev1 - зима, тек - весна
        prevYear3 = prevYear2 - 1;
    } else {
        prevYear3 = prevYear2;
    }
    ...
    // Добавляем предыдущие 3 сезона
    var prevSeason1 = seasonsOrder[prevSeasonIndex1];
    seasons.push({
        code: prevSeason1 + '_' + prevYear1,
        title: seasonTranslations[prevSeason1] + ' ' + prevYear1
    });

    var prevSeason2 = seasonsOrder[prevSeasonIndex2];
    seasons.push({
        code: prevSeason2 + '_' + prevYear2,
        title: seasonTranslations[prevSeason2] + ' ' + prevYear2
    });

    var prevSeason3 = seasonsOrder[prevSeasonIndex3];
    seasons.push({
        code: prevSeason3 + '_' + prevYear3,
        title: seasonTranslations[prevSeason3] + ' ' + prevYear3
    });

    // Возвращаем массив с 4 сезонами
    return seasons;
}
    ...
    function Component() {
        // Добавление дополнительной информации из Shikimori
